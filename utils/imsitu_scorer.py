...
class imsitu_scorer():
  ...
  def add_point_both(self, pred_verbs, verbs, pred_roles_nouns, roles_nouns, gt_pred_roles_nouns):
    ...
    for i in range(batch_size):
      ...
      pred_role_noun = pred_roles_nouns[i]
      gt_pred_role_noun = gt_pred_roles_nouns[i]

      role_noun = roles_nouns[i]

      _, pred_verb_idx = torch.topk(pred_verb, self.topk)
      _, pred_role_noun_idx = torch.topk(pred_roles_nouns, self.topk)      
      ...
      for k in range(0, self.topk):
        ...
        for r in range(0, gt_roles_count):
          ...
          for n in range(0, 3):
            if pred_role_noun_idx[0][r][k] == role_noun[n][r]:
              found += 1
        
        if found == (3 * gt_roles_count):
          all_found = True
      ...
      if self.topk == 1:
        for r in range(0, gt_roles_count):
          #for all nouns in three annotations
          for n in range(0, 3):
            if gt_pred_role_noun_idx[0][r][0] == role_noun[n][r]:
              gt_found += 1
        
        print(gt_found, " == ", gt_roles_count*3)
        if gt_found == (3 * gt_roles_count):
            gt_all_found = True
