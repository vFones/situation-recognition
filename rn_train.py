...
def train(model, train_loader, dev_loader, optimizer, scheduler, max_epoch, encoder, model_name, model_saving_name, checkpoint=None):
  ...
  for e in range(epoch, max_epoch+1):
    ...
    for i, (_, img, verb, nouns) in enumerate(train_loader):
      ...
      optimizer.zero_grad()
      pred_nouns = model(img)

      nouns_t = nouns.transpose(1, 2).contiguous().view(img.size(0) * encoder.max_role_count*3, -1)
      pred_nouns = pred_nouns.contiguous().view(img.size(0)*encoder.max_role_count, -1)
      pred_nouns.expand(3, pred_nouns.size(0), pred_nouns.size(1))
      pred_nouns = pred_nouns.transpose(0, 1)
      pred_nouns = pred_nouns.contiguous().view(-1, pred_nouns.size(-1))
      print(nouns, nouns_t.size())
      print(pred_nouns, pred_nouns.size())

      lossfn = torch.nn.CrossEntropyLoss()
      
      loss = lossfn(pred_nouns, nouns_t)
      loss.backward()
